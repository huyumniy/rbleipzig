...
import { getAvailableBlockIds, findNearbyChains, getBiggestChainSlice } from "../utils/filtration.js";
...
export async function main() {
  try {
    while (true) {
      ...
      let categoriesDict = settings.matchesData.filter(match => current_match_name === match.name);
      ...
      let availableBlockIds = getAvailableBlockIds(areasResponse, categoriesDict);
      ...
      let randomCategory = availableBlockIds[Math.floor(Math.random() * availableBlockIds.length)]

      let randomBlockId = randomCategory.blockIds[Math.floor(Math.random() * randomCategory.blockIds.length)]


      // Fetch seat data
      const seatsEndpoint = `${domain}/tnwr/v1/seatmap/seats/free/3ddvBlock` +
      ...
      let nearbyChains = findNearbyChains(seatsResponse.features, randomCategory.quantity, randomCategory.category, blacklistIds)
      ...
      let randomChainSlice = getBiggestChainSlice(nearbyChains, randomCategory.quantity)
      ...
      const items = Array.from(grouptedItems.values()).map(item => ({
        ...item,
        quantity: item.physicalSeatIds.length
      }));
      ...
      if (cartResponse?.errors?.some( err => err?.errorCode === "TOO_MANY_TICKETS")) {
